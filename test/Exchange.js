    ...
    const feePercent = 10
        ...
        describe('Checking Balances', () => {
            ...
            describe('Order actions', async () => {
                ...
                let amount = tokens(1)
                beforeEach(async () =>{
                    //user 1 deposit before making an order
                    transaction = await token1.connect(user1).approve(exchange.address, amount)
                    result = await transaction.wait()
                    transaction = await exchange.connect(user1).depositToken(token1.address, amount)
                    result = await transaction.wait()
                    ...
                    //user 2 deposit before filling an order
                    ...
                    //make order
                    transaction = await exchange.connect(user1).makeOrder(token2.address, tokens(1), token1.address, amount)
                    result = await transaction.wait()
                })
